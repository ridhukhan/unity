...
import Rinmember2 from "@/models/Rinmember2";
...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    const updatedMember = await Rinmember2.findByIdAndUpdate(id, body, {
      new: true,
      runValidators: true,
    });
...
export async function DELETE(req, { params }) {
  ...
  try {
    await dbConnect();
    const { id } = await params;
    const deletedMember = await Rinmember2.findByIdAndDelete(id);
